...
@router.post("/add-processing-fee")
def add_processing_fee(request: Request, data: ProcessingFeeCreate):
    user_state = getattr(request.state, "user", {})
    response = credit_service.add_processing_fee(user_id=user_state.get("id"), form_data=data)
    return ApiResponse.create_response(
        success=response.get("success"),
        message=response.get("message"),
        status_code=response.get("status_code", status.HTTP_200_OK),
...
@router.put("/update-processing-fee/{fee_id}")
def update_processing_fee(request: Request, fee_id: int, data: ProcessingFeeUpdate):
    user_state = getattr(request.state, "user", {})
    response = credit_service.update_processing_fee(user_id=user_state.get("id"), fee_id=fee_id, form_data=data)
    return ApiResponse.create_response(
        success=response.get("success"),
        message=response.get("message"),
        status_code=response.get("status_code", status.HTTP_200_OK),
...
@router.get("/get-all-processing-fee")
def get_all_processing_fee(request: Request):
    user_state = getattr(request.state, "user", {})
    response = credit_service.get_all_processing_fees(user_id=user_state.get("id"))
    return ApiResponse.create_response(
        success=response.get("success"),
        message=response.get("message"),
        status_code=response.get("status_code", status.HTTP_200_OK),
...
@router.get("/get-processing-fee-details/{fee_id}")
def get_processing_fee_details(request: Request, fee_id: int):
    user_state = getattr(request.state, "user", {})
    response = credit_service.get_processing_fee_detail(user_id=user_state.get("id"), fee_id=fee_id)
    return ApiResponse.create_response(
        success=response.get("success"),
        message=response.get("message"),
        status_code=response.get("status_code", status.HTTP_200_OK),
